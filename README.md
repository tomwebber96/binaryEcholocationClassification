...
Use the batch script *runClassifier_PGDF.bat* to work from .pgdf's directly. Currently individual predictions can be stored as .npz files with one .npz file per .pgdf file. .npz files can also be opened within R (https://cran.r-project.org/web/packages/RcppCNPy/vignettes/UsingReticulate.pdf) depending on the users workflow preferences.  

Run the runClassifier_PGDF.bat. It will ask for which model you want to select, 96kHz or 250kHz. Select option 1 or 2. Any errors should become obvious and most likely due to the python installation on the system. The .bat will also ask if you want to save the individual predictions to .npz files, and what PAMGuard click detection threshold (signal excess) you with to use. The current PAMGuard .psfx has a default click detection threshold of 10dB. The .bat file allows 10, 12, 14, and 16 dB thresholds to be implimented by predicting and classifying based on clicks which meet these thresholds. These different thresholds were chosen to enable classiifcation to be limited which can be useful in particulalry noisey or shallow environments. The specific dB threshold values can be edited within the .py file if required.

The .runClassPGDF currently works with a pypamguard_Patch.py to import the .pgdf waveforms as full double, where pypamguard (v1.0.1) rounds waveform values to 4 decimal places, which initially impacted prediction scores, though minor.
 
The .bat file will create a new folder called classified with a copy of the .npz files, containing the individual prediction scores along with UIDs and clipped waveforms. It will also generate a .csv file with a mean classification score in 5-minute time bins, along with total number of clicks, number of positive, and number of negative (based on <0.5 being negative).
