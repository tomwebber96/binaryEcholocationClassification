...
Next, run createVirEnv.bat. It will ask to be directed to your python installation. If unsure run *where python* in cmd or your anaconda installation. Python's Pip installation may require admin rights. For use with NVIDIA GPUs, install CUDA ([11.7] https://developer.nvidia.com/cuda-11-7-0-download-archive) and cuDNN ([8.9] https://developer.nvidia.com/rdp/cudnn-archive). These versions have been tested and work with the current setup, but if a different GPU or other versions of packages are installed, further installs/ updates maybe required for your specific needs. 
